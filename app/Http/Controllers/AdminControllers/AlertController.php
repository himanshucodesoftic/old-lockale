...
class AlertController extends Controller
{
    ...
    public function vendorRequestStatus($requestData) {
        if(!empty($requestData['request_email'])) {
            Mail::send('/mail/requestStatus', ['requestData' => $requestData], function($m) use ($requestData){
                $m->to($requestData['request_email'])->subject(Lang::get("labels.RequestStatus"))->getSwiftMessage()
                ->getHeaders()
                ->addTextHeader('x-mailgun-native-send', 'true');
            });
        }
    ...
    public function carRequestStatus($requestData) {        
        if(!empty($requestData['request_email'])) {
            Mail::send('/mail/carStatus', ['requestData' => $requestData], function($m) use ($requestData){
                $m->from('admin@gmail.com', 'LokalKW');
                $m->to($requestData['request_email'])->subject(Lang::get("labels.RequestStatus"))->getSwiftMessage()
                ->getHeaders()
                ->addTextHeader('x-mailgun-native-send', 'true');
            });
