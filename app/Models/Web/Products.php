...
{
    use Sortable;
    public $sortable =['products_id','created_at'];
    ...
    public function postProduct($request) {
        ...
        if (empty($request['images'])) {
            ...
            $uploadImage = $request['images'][0]['id'];
        }        
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function products($data)
    {
        ...
        if (count($products) > 0) {
            ...
            foreach ($products as $products_data) {
                ...
                $products_data->avarage_rate = round($avarage_rate);
                ...
                $prods_images = DB::table('products_images')
                    ->LeftJoin('image_categories', 'products_images.image', '=', 'image_categories.image_id')
                    ->select('image_categories.path as image_path', 'image_categories.image_type')
                    ->where('products_id', '=', $products_id)
                    ->where('image_type', '=', 'ACTUAL')
                    ->orderBy('sort_order', 'ASC')
                    ->get();
                if (count($prods_images) > 0) {
                    $products_data->hover_img = $prods_images[0]->image_path;
                } else {
                    $products_data->hover_img = '';
                }
    ...
    public function getmyproduct($request) {
        ...
        'specials.expires_date', 'image_categories.path as path', 'products.updated_at as productupdate',  'products.created_at as productcreated','categories_description.categories_id',
        ...
        $products = $data->orderBy('products.products_id', 'DESC')
            ...
            ->groupBy('products.products_id')->paginate(12);
