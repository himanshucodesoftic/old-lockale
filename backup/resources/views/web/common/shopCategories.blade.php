    ...
    if ($categories) {
        ...
        foreach ($categories as $parents) {
            ...
            if (isset($parents->childs)) {
                ...
                $option .= '<a class="main-manu shownd"' . $hasChild . '>' . $parents->categories_name . '</a>';
            } else {
                $hasChild = "href=" . url('shop?category=') . $parents->slug;
                $option .= '<a class="main-manu"' . $hasChild . '>' . $parents->categories_name . '</a>';
            }
            // <img class="img-fuild" src="' . asset($parents->path) . '">
            // $option .= '<a class="main-manu active"' . $hasChild . '>' . $parents->categories_name . '</a>';

            if (isset($parents->childs)) {
                $total = '';
                $i = 1;
                $option .= ' <div class="collapse" id="' . $parent_slug . '">
...
function shopChildcat($childs, $i, $parent_id)
{
    ...
    foreach ($childs as $key => $child) {
        ...
        $contents .= '<li class="list-item" style="margin-top: 8px;">
